...
const ActiveChannelsEnhancer = {
    ...
    init() {
        ...
        this.updateInterval = setInterval(async () => this.fetchAndUpdateChannels(), 60000);
    },
    ...
    async fetchAndUpdateChannels() {
        ...
        this.waitForChannelsAndReplace();
    },
        ...
        api.storage.local.get(['sessionId'], (result) => {
            ...
            if (result.sessionId && userId) {
                fetch(`${CONFIG.apiUrl}/activity/update`, {
                    ...
                    body: JSON.stringify({ sessionId: result.sessionId, userId: userId })
                })
                    .then(response => {
                        if (!response.ok) {
                            throw new Error('Сервер вернул ошибку: ' + response.status);
