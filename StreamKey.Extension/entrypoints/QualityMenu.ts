...
export class QualityMenu {
    ...
    async autoSwitch(): Promise<void> {
        ...
        const currentSelected = elements.find((label) => {
            ...
            const input = radioItem.querySelector<HTMLInputElement>("input[type='radio']");
        ...
        const input = targetResolution.radioItem?.querySelector<HTMLInputElement>("input[type='radio']");

        if (input && !input.disabled) {
            input.click();
            console.log(`Auto-switched from 1440p to ${targetResolution.resolution}p`);
        }
    }
